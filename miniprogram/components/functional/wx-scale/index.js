    ...
    draw(num, total, {
      ...
      for (let i = 0; i < rul.unitNum + 1; i++) {
        ...
        if (i % (rul.single / rul.step) === 0) {
          ...
          if (self.data.int) {
            ctx.fillText(i * rul.step + rul.minNum + 'cm', rul.FIXED_NUM / 2 + (i * rul.spa), canvasHeight - 15)
          } else {
            ctx.fillText(i / (rul.single / rul.step) + rul.minNum + 'cm', rul.FIXED_NUM / 2 + (i * rul.spa), canvasHeight - 15)
          }
        }
        // 绘制刻度
        if (i % 5 === 0) {
